...
$soma_totalBanco = $stmt_sum_total->fetchColumn();
if($soma_totalBanco <= 0){
    $soma_total = 0;
} else {
    $soma_total = $soma_totalBanco;
}
// media
$sql_media = "SELECT media FROM analise where codigo = '".$_SESSION['user']."' ";
$stmt_media = $PDO->prepare($sql_media);
$stmt_media->execute();
$mediaBaco = $stmt_media->fetchColumn();
if($mediaBaco <= 0){
    $media = 0;
} else {
    $media = $mediaBaco;
...
$stmt_porcen->execute();
$porcenBanco = $stmt_porcen->fetchColumn();
if($porcenBanco <= 0){
    $porcen = 0;
} else {
    $porcen = $porcenBanco;
...
//hora de atualizacao grafico
$sql_id = "SELECT id FROM atzconf where codigo = '$user_colaborador' ";
$stmt_id = $PDO->prepare($sql_id);
$stmt_id->execute();
$id = $stmt_id->fetchColumn();

sleep(2);
$sql_del = "DELETE FROM atzconf where id = '".$id."' ";
$stmt_del = $PDO->prepare($sql_del);
$stmt_del->execute();

$hora = date("H:i");

$sql = "INSERT INTO  atzconf (codigo, nome, total, media, porcen, micro, hora) value(:codigo, :nome, :total, :media, :porcen, :micro, :hora)";
$stmt = $PDO->prepare($sql);
$stmt->bindParam(':codigo', $user_colaborador);
$stmt->bindParam(':nome', $nome_colaborador);
$stmt->bindParam(':total', $soma_total);
$stmt->bindParam(':media', $media);
$stmt->bindParam(':porcen', $porcen);	
$stmt->bindParam(':micro', $grupo);
        ...
        <div class="container" align="center">
            <div class="container" style="width: 90%;">
                <div class="border border-danger"
                    style="margin-top: 40px; background-color: white; color: black; width: 90%; border-radius: 20px; ">
                    <br>
                    <div class="row justify-content-start">
                        <div class="col-md-2 mb-2" style="font-size: 12px;">
                            <label type="text" class="form-control"
                                style="text-align: center; font-size: 12px;">Grupo</label>
                            <label type="text" class="form-control"
                                style="text-align: center; font-size: 12px;"><?php echo $grupo ?></label>
                        </div>
