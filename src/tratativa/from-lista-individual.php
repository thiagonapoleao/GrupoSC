...
// SQL para selecionar os registros
$sql_arry = "SELECT data, cod, nome, SUM(erro) total, SUM(case when motivo='1- Falta' then erro end) falta, SUM(case when motivo='2- Sobra' then erro end) sobra,  SUM(case when motivo='3- Troca' then erro end) troca,  SUM(case when motivo='4- Erro Conf' then erro end) conf FROM hist0040 where cod = $codigo GROUP BY data,cod,nome order by data DESC, nome ASC";
// seleciona os registros
...
<body style="background-image:  url('../img/TelanovaGrupoSC.png'); background-repeat: no-repeat; background-attachment: fixed; background-size: 100% 100%; ">
    ...
    <div class="container" align="center" style="min-height: 500px;">
        <div class="border border-danger" style="background-color: white; color: black; width: 70%; border-radius: 20px; margin-top: 80px; font-size: 12px;">
            <from>
                <h4 align="center">Erros Detalhados por Separador</h4>
